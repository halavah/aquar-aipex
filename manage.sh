...
show_menu() {
    ...
    echo -e "${GREEN}  2.${NC} 🚀 ${BLUE}启动后端服务器${NC}     (start-backend.sh)"
    echo -e "     ${PURPLE}→${NC} 启动 Spring Boot 后端"
    echo ""

    echo -e "${GREEN}  3.${NC} 🎨 ${BLUE}启动前端服务器${NC}     (start-frontend.sh)"
    echo -e "     ${PURPLE}→${NC} 启动 Vue 前端开发环境"
    echo ""

    echo -e "${GREEN}  4.${NC} 🧹 ${BLUE}清理 Git 跟踪${NC}     (ignore.sh)"
...
main() {
    while true; do
        ...
        case $choice in
            ...
            3)
                print_header "执行: 启动前端服务器"
                run_script "start-frontend.sh"
                ;;
